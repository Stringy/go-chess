...
func InitialiseAllMasks() {
	...
	MaskEG[0] = BitSet[C1] | BitSet[D1] | BitSet[E1]
	MaskEG[1] = BitSet[C8] | BitSet[D8] | BitSet[E8]
	...
	move.SetFrom(E1)
	move.SetTo(G1)
	WhiteCastleOO = move.Val
	move.SetTo(C1)
	WhiteCastleOOO = move.Val

	move.SetPiece(BlackKing)
	move.SetProm(BlackKing)
	move.SetFrom(E8)
	move.SetTo(G8)
	BlackCastleOO = move.Val
	move.SetTo(C8)
	BlackCastleOOO = move.Val
}
...
func initialiseGeneralSlidingAttacks() {	
	for sq := 0; sq < 8; sq++ {
		for state6 := 0; state6 < 64; state6++ {
			...
			attBit := 0
			if sq > 7 {
				attBit |= int(CharBitSet[sq + 1])
			}
			slide := sq + 2
			for ; slide < 8; {
				if (int(CharBitSet[slide - 1]) &^ state8) != 0 {
					attBit |= int(CharBitSet[slide])
				} else {
					break
				}
				slide++
			...
			GeneralSlidingAttacks[sq][state6] = uint64(attBit) //possible dodgy cast (expected loss of data)
		}
	}
}
...
func initialiseFileAttacks(sq, state int) {
	...
	for attbit := 0; attbit < 8; attbit++ {
		if (GeneralSlidingAttacks[8 - Ranks[sq]][state] & uint64(CharBitSet[attbit])) != 0 {
...
func initialiseA1H8Masks(sq, state int) {
	...
	for attbit := 0; attbit < 8; attbit++ {
		if Ranks[sq] - 1 < Files[sq] - 1 {
			if (GeneralSlidingAttacks[Ranks[sq] - 1][state] & uint64(CharBitSet[attbit])) != 0 {
			...
			if (GeneralSlidingAttacks[Files[sq] - 1][state] & uint64(CharBitSet[attbit])) != 0 {
				diag := Files[sq] - Ranks[sq]
				var file, rank int
				if diag > 0 {
					file = attbit + 1
